...
IR *CreateStatement::translate(vector<IR *> &v_ir_collector) {
  ...
  res = new IR(kCreateStatement, OP3("", "(", ")"), res, tmp3);
  CASEEND
  ...
  res = new IR(kCreateStatement, OP3("", "AS", ""), res, tmp3);
  CASEEND
  ...
  PUSH(res);
  auto tmp4 = SAFETRANSLATE(ColumnDefListNoType_);
...
IR *ColumnDef::translate(vector<IR *> &v_ir_collector) {
  ...
  res = new IR(kColumnDeftop, OP0(), res, tmp);
...
IR *OptOrder::translate(vector<IR *> &v_ir_collector) {
  ...
  CASESTART(0)
  res = SAFETRANSLATE(order_);
  res = new IR(kOptOrder, OP0(), res);
  CASEEND
...
IR *BinaryExpr::translate(vector<IR *> &v_ir_collector) {
  ...
  auto tmp2 = SAFETRANSLATE(FtsQueryLiteral_);
  res = new IR(kBinaryExpr, OPMID(operator_), tmp1, tmp2);
  CASEEND
...
IR *FunctionExpr::translate(vector<IR *> &v_ir_collector) {
  ...
  CASESTART(2)
  auto tmp = SAFETRANSLATE(WindowFuncNoArg_);
  auto tmp1 = SAFETRANSLATE(OverClauseWithOrder_);
  res = new IR(kFunctionExpr, OP3("", "( )", ""), tmp, tmp1);
  CASEEND

  CASESTART(3)
  auto tmp = SAFETRANSLATE(WindowFunc_);
  auto tmp1 = SAFETRANSLATE(expr_list_);
  auto tmp2 = SAFETRANSLATE(OverClause_);
  res = new IR(kFunctionExpr, OP3("", "(", ")"), tmp, tmp1);
  PUSH(res);
  res = new IR(kFunctionExpr, OP0(), res, tmp2);
  CASEEND

  // CASESTART(4)
  // auto tmp = SAFETRANSLATE(AggregationFunc_);
  // auto tmp1 = SAFETRANSLATE(opt_filter_clause_);
  // auto tmp2 = SAFETRANSLATE(OverClauseWithOrder_);
  // res = new IR(kFunctionExpr, OP3("", "(", ")"), tmp, tmp1);
  // PUSH(res);
  // res = new IR(kFunctionExpr, OP0(), tmp1, tmp2);
  // CASEEND
  ...
  // auto tmp2 = SAFETRANSLATE(expr_list_);
  // auto tmp3 = SAFETRANSLATE(opt_filter_clause_);
  // auto tmp4 = SAFETRANSLATE(OverClauseWithOrder_);
  // res = new IR(kFunctionExpr, OP3("", "(", ""), tmp, tmp1);
  // PUSH(res);
  // res = new IR(kFunctionExpr, OP3("", ")", ""), res, tmp2);
  // PUSH(res);
  // res = new IR(kFunctionExpr, OP0(), res, tmp3);
  // PUSH(res);
  // res = new IR(kFunctionExpr, OP0(), res, tmp4);
  // CASEEND

  CASESTART(4)
  res = SAFETRANSLATE(AggregationFunc_);
  res = new IR(kFunctionExpr, OPEND("()"), res);
  PUSH(res);
  auto tmp1 = SAFETRANSLATE(opt_filter_clause_);
  res = new IR(kFunctionExpr, OP0(), res, tmp1);
...
IR *json_literal::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  res = new IR(kjson_literal, "" + str_val_ + "");
  TRANSLATEEND
}

IR *json_pathliteral::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  res = new IR(kjson_pathliteral, "'$" + str_val_ + "'");
  TRANSLATEEND
}
...
IR *OptWithClause::translate(vector<IR *> &v_ir_collector) {
  ...
  CASESTART(1)
  res = SAFETRANSLATE(WithRecursiveClause_);
  res = new IR(kOptWithClause, OP0(), res);
  CASEEND
...
void FunctionExpr::deep_delete() {
  ...
  SAFEDELETE(WindowFuncNoArg_);
  SAFEDELETE(OverClauseWithOrder_);
  SAFEDELETE(WindowFunc_);
  SAFEDELETE(OverClause_);
  SAFEDELETE(AggregationFunc_);
  delete this;
}
...
void json_literal::deep_delete() { delete this; }

void json_pathliteral::deep_delete() { delete this; }
...
void OptWithClause::deep_delete() {
  ...
  SAFEDELETE(WithRecursiveClause_);
  delete this;
}
...
IR *ColumnArg::translate(vector<IR *> &v_ir_collector) {
  ...
  auto tmp2 = new IR(koptPrimarykey, OP1("PRIMARY KEY"), tmp, tmp1);  // modify
  res = new IR(kColumnArg, OP0(), tmp2);
  PUSH(res);
  ...
  CASESTART(4)
  res = SAFETRANSLATE(expr_);
  auto tmp = SAFETRANSLATE(opt_generatedarg_);
  res = new IR(kColumnArg, OP3("GENERATED ALWAYS AS(", ")", ""), res, tmp);
  CASEEND
  // CASESTART(5)
  // res = SAFETRANSLATE(expr_);
...
IR *OptOverClause::translate(vector<IR *> &v_ir_collector) {
  ...
  CASESTART(0)
  res = SAFETRANSLATE(OverClause_);
  res = new IR(kOptOverClause, OP0(), res);
  CASEEND
...
IR *OptFilterClause::translate(vector<IR *> &v_ir_collector) {
  ...
  CASESTART(0)
  res = SAFETRANSLATE(expr_);
  res = new IR(kOptFilterClause, OP2("FILTER( WHERE", ")"), res);
  CASEEND
  CASESTART(1)
...
IR *Window::translate(vector<IR *> &v_ir_collector) {
  ...
  CASESTART(0)
  auto tmp = SAFETRANSLATE(WindowWithOrder_);
  res = new IR(kWindow, OP0(), tmp);
  CASEEND

  CASESTART(1)
  auto tmp = SAFETRANSLATE(WindowNoOrder_);
  res = new IR(kWindow, OP0(), tmp);
  CASEEND
...
void Window::deep_delete() {
  SAFEDELETE(WindowWithOrder_);
  SAFEDELETE(WindowNoOrder_);
  delete this;
}
...
IR *ColumnInParen_ForeignKey::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp0 = SAFETRANSLATE(column_name_list_ForeignKey_);
  res = new IR(kColumnInParen_ForeignKey, OP3("(", "", ")"), tmp0);
  TRANSLATEEND
}

void ColumnInParen_ForeignKey::deep_delete() {
  SAFEDELETE(column_name_list_ForeignKey_);
  delete this;
}

IR *ColumnNameList_ForeignKey::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  TRANSLATELIST(kColumnNameList_ForeignKey, v_column_name_list_ForeignKey_,
                ",");
  TRANSLATEENDNOPUSH
}

void ColumnNameList_ForeignKey::deep_delete() {
  SAFEDELETELIST(v_column_name_list_ForeignKey_);
  delete this;
}

IR *ColumnName_ForeignKey::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  SWITCHSTART
  CASESTART(0)
  res = SAFETRANSLATE(identifier_);
  res = new IR(kColumnName_ForeignKey, OP0(), res);
  CASEEND
  SWITCHEND
  TRANSLATEEND
}

void ColumnName_ForeignKey::deep_delete() {
  SAFEDELETE(identifier_);
  delete this;
}

IR *ColumnInParen_References::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp0 = SAFETRANSLATE(column_name_list_References_);
  res = new IR(kColumnInParen_Refernecs, OP3("(", "", ")"), tmp0);
  TRANSLATEEND
}

void ColumnInParen_References::deep_delete() {
  SAFEDELETE(column_name_list_References_);
  delete this;
}

IR *ColumnNameList_References::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  TRANSLATELIST(kColumnNameList_References, v_column_name_list_References_,
                ",");
  TRANSLATEENDNOPUSH
}

void ColumnNameList_References::deep_delete() {
  SAFEDELETELIST(v_column_name_list_References_);
  delete this;
}
...
IR *ColumnName_References::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  SWITCHSTART
  CASESTART(0)
  res = SAFETRANSLATE(identifier_);
  res = new IR(kColumnName_References, OP0(), res);
  CASEEND
  SWITCHEND
  TRANSLATEEND
}

void ColumnName_References::deep_delete() {
  SAFEDELETE(identifier_);
  delete this;
}
...
IR *ForeignKeyRef::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp = SAFETRANSLATE(table_name_);
  auto tmp1 = SAFETRANSLATE(column_name_References_);
...
void ForeignKeyRef::deep_delete() {
  SAFEDELETE(table_name_);
  SAFEDELETE(column_name_References_);
  delete this;
}
...
IR *Jsonexpr_all::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp = SAFETRANSLATE(Jsondata_);
  auto tmp1 = SAFETRANSLATE(Jsonpath_);
  res = new IR(kJsonexpr_all, OP3("", "->", ""), tmp, tmp1);
  TRANSLATEEND
}

void Jsonexpr_all::deep_delete() {
  SAFEDELETE(Jsondata_);
  SAFEDELETE(Jsonpath_);
  delete this;
}

IR *Jsondata::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  SWITCHSTART
  CASESTART(0)
  auto tmp = SAFETRANSLATE(json_literal_);
  res = new IR(kJsondata, OP2("", ""), tmp);
  CASEEND
  CASESTART(1)
  auto tmp1 = SAFETRANSLATE(column_name_);
  res = new IR(kJsondata, OP2("", ""), tmp1);
  CASEEND
  SWITCHEND
  TRANSLATEEND
}

void Jsondata::deep_delete() {
  SAFEDELETE(json_literal_);
  SAFEDELETE(column_name_);
  delete this;
}

IR *Jsonpath::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp = SAFETRANSLATE(json_pathliteral_);
  res = new IR(kJsonpath, OP2("", ""), tmp);
  TRANSLATEEND
}

void Jsonpath::deep_delete() {
  SAFEDELETE(json_pathliteral_);
  delete this;
}

IR *opt_generatedarg::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  SWITCHSTART
  CASESTART(0)
  res = new IR(kopt_generatedarg, str_val_);
  CASEEND
...
void opt_generatedarg::deep_delete() { delete this; }

IR *JsonExpr::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp1 = SAFETRANSLATE(Jsonexpr_all_);
  res = new IR(kJsonExpr, OP0(), tmp1);
  TRANSLATEEND
}

void JsonExpr::deep_delete() {
  SAFEDELETE(Jsonexpr_all_);
  delete this;
}

IR *WithRecursiveClause::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART

  res = SAFETRANSLATE(with_rec_desc_list);
  res = new IR(kWithRecursiveClause, OPSTART("WITH RECURSIVE"), res);

  TRANSLATEEND
}

void WithRecursiveClause::deep_delete() {
  SAFEDELETE(with_rec_desc_list);
  delete this;
}

IR *WithRecDescList::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART

  TRANSLATELIST(kWithRecDescList, v_with_rec_desc_list, ",");

  TRANSLATEENDNOPUSH
}

void WithRecDescList::deep_delete() {
  SAFEDELETELIST(v_with_rec_desc_list);
  delete this;
}

IR *WithRecDesc::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp = SAFETRANSLATE(id_);
  auto tmp1 = SAFETRANSLATE(with_recursive_select_);
  res = new IR(kWithRecDesc, OP3("", "AS", ""), tmp, tmp1);
  TRANSLATEEND
}

void WithRecDesc::deep_delete() {
  SAFEDELETE(id_);
  SAFEDELETE(with_recursive_select_);
  delete this;
}

IR *WithRecursiveSelect::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp = SAFETRANSLATE(init_select_);
  auto tmp1 = SAFETRANSLATE(opt_recursive_select_);
  res = new IR(kWithRecursiveSelect, OP3("(", "", ")"), tmp, tmp1);
  TRANSLATEEND
}

void WithRecursiveSelect::deep_delete() {
  SAFEDELETE(init_select_);
  SAFEDELETE(opt_recursive_select_);
  delete this;
}

IR *OptRecursiveSelect::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  SWITCHSTART
  CASESTART(0)
  auto tmp1 = SAFETRANSLATE(opt_distinct_);
  auto tmp2 = SAFETRANSLATE(select_list_);
  auto tmp3 = SAFETRANSLATE(opt_from_clause_);
  auto tmp4 = SAFETRANSLATE(opt_where_);
  auto tmp5 = SAFETRANSLATE(opt_group_);
  auto tmp6 = SAFETRANSLATE(opt_order_);
  res = new IR(kUnknown, OPSTART("UNION ALL SELECT"), tmp1, tmp2);
  PUSH(res);
  res = new IR(kUnknown, OP0(), res, tmp3);
  PUSH(res);
  res = new IR(kUnknown, OP0(), res, tmp4);
  ...
  res = new IR(kUnknown, OP0(), res, tmp5);
  PUSH(res);
  res = new IR(kOptRecursiveSelect, OP3("", "", "LIMIT 10"), res, tmp6);
  CASEEND

  CASESTART(1)
  res = new IR(kOptRecursiveSelect, OP0());
  CASEEND
  SWITCHEND
  TRANSLATEEND
}

void OptRecursiveSelect::deep_delete() {
  SAFEDELETE(opt_distinct_);
  SAFEDELETE(select_list_);
  SAFEDELETE(opt_from_clause_);
  SAFEDELETE(opt_where_);
...
IR *InitSelect::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp1 = SAFETRANSLATE(opt_distinct_);
  auto tmp2 = SAFETRANSLATE(select_list_essential_alias_);
  auto tmp3 = SAFETRANSLATE(opt_from_clause_);
  auto tmp4 = SAFETRANSLATE(opt_where_);
  auto tmp5 = SAFETRANSLATE(opt_group_);
  res = new IR(kUnknown, OPSTART("SELECT"), tmp1, tmp2);
  ...
  res = new IR(kUnknown, OP0(), res, tmp4);
  PUSH(res);
  res = new IR(kInitSelect, OP0(), res, tmp5);
  TRANSLATEEND
}

void InitSelect::deep_delete() {
  SAFEDELETE(opt_distinct_);
  SAFEDELETE(select_list_essential_alias_);
  SAFEDELETE(opt_from_clause_);
  SAFEDELETE(opt_where_);
  SAFEDELETE(opt_group_);
  delete this;
...
IR *SelListAlias::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART

  res = SAFETRANSLATE(expr_list_essential_alias_);
  res = new IR(kSelListAlias, OP0(), res);

  TRANSLATEEND
}

void SelListAlias::deep_delete() {
  SAFEDELETE(expr_list_essential_alias_);
  delete this;
}

IR *ExprListAlias::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART

  TRANSLATELIST(kExprListAlias, v_expr_list_essential_alias_, ",");

  TRANSLATEENDNOPUSH
}

void ExprListAlias::deep_delete() {
  SAFEDELETELIST(v_expr_list_essential_alias_);
  delete this;
}

IR *ExprEssentialAlias::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp1 = SAFETRANSLATE(expr_);
  auto tmp2 = SAFETRANSLATE(alias_);
  res = new IR(kExprEssentialAlias, OP0(), tmp1, tmp2);
  TRANSLATEEND
}

void ExprEssentialAlias::deep_delete() {
  SAFEDELETE(expr_);
  SAFEDELETE(alias_);
  delete this;
}

IR *FtsQueryLiteral::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  res = new IR(kFtsQueryLiteral, "'" + str_val_ + "'");
  TRANSLATEEND
}

void FtsQueryLiteral::deep_delete() { delete this; }

IR *WindowWithOrder::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART

  auto windowname = SAFETRANSLATE(opt_base_window_name_);
  auto order = SAFETRANSLATE(order_);
  ...
  PUSH(tmp);
  tmp = new IR(kUnknown, OP0(), tmp, order);
  PUSH(tmp);
  res = new IR(kWindow, OP0(), tmp, opt_frame);
  CASEEND
  CASESTART(1)
  auto tmp = new IR(kUnknown, OP0(), windowname, order);
  PUSH(tmp);
  res = new IR(kWindowWithOrder, OP0(), tmp, opt_frame);
  CASEEND
  SWITCHEND
...
void WindowWithOrder::deep_delete() {
...
IR *WindowNoOrder::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  ...
  CASEEND
  CASESTART(1)
  res = new IR(kWindowNoOrder, OP0(), windowname, opt_frame);
  CASEEND
  SWITCHEND
...
void WindowNoOrder::deep_delete() {
  SAFEDELETE(opt_base_window_name_);
  SAFEDELETE(expr_list_);
  SAFEDELETE(opt_frame_);
  delete this;
...
IR *Order::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART

  res = SAFETRANSLATE(order_list_);
  res = new IR(kOrder, OP1("ORDER BY"), res);

  TRANSLATEEND
}

void Order::deep_delete() {
  SAFEDELETE(order_list_);
  delete this;
}

IR *OverClause::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  SWITCHSTART
  CASESTART(0)
  res = SAFETRANSLATE(OverClauseWithOrder_);
  res = new IR(kOverClause, OP0(), res);
  CASEEND
  CASESTART(1)
  res = SAFETRANSLATE(OverClauseNoOrder_);
  res = new IR(kOverClause, OP0(), res);
  CASEEND
  CASESTART(2)
  res = SAFETRANSLATE(id_);
  res = new IR(kOverClause, OP1("OVER"), res);
  CASEEND
...
void OverClause::deep_delete() {
  SAFEDELETE(OverClauseWithOrder_);
  SAFEDELETE(OverClauseNoOrder_);
  SAFEDELETE(id_);
  delete this;
}

IR *OverClauseWithOrder::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART

  auto tmp = SAFETRANSLATE(WindowWithOrder_);
  res = new IR(kOverClauseWithOrder, OP2("OVER (", ")"), tmp);

  TRANSLATEEND
}

void OverClauseWithOrder::deep_delete() {
  SAFEDELETE(WindowWithOrder_);
  delete this;
}

IR *OverClauseNoOrder::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
...
void OverClauseNoOrder::deep_delete() {
  SAFEDELETE(WindowNoOrder_);
  delete this;
}

IR *WindowFuncNoArg::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  res = new IR(kWindowFuncNoArg, str_val_);
  TRANSLATEEND
}

void WindowFuncNoArg::deep_delete() { delete this; }

IR *WindowFunc::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  res = new IR(kWindowFunc, str_val_);
  TRANSLATEEND
}

void WindowFunc::deep_delete() { delete this; }

IR *AggregationFunc::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  res = new IR(kAggregationFunc, str_val_);
  TRANSLATEEND
}

void AggregationFunc::deep_delete() { delete this; }

IR *ColumnDefListNoType::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART

  TRANSLATELIST(kColumnDefListNoType, v_column_def_no_type_list_, ",");

  TRANSLATEENDNOPUSH
}

void ColumnDefListNoType::deep_delete() {
  SAFEDELETELIST(v_column_def_no_type_list_);
  delete this;
}

IR *ColumnDefNoType::translate(vector<IR *> &v_ir_collector) {
  TRANSLATESTART
  auto tmp = SAFETRANSLATE(id_);
  res = new IR(kColumnDefNoType, OP0(), tmp);
  TRANSLATEEND
}

void ColumnDefNoType::deep_delete() {
  SAFEDELETE(id_);
  delete this;
}
...
std::string get_string_by_idtype(IDTYPE type){
    static const std::unordered_map<IDTYPE, std::string> idtype_map = {
        {id_whatever, "id_whatever"},

        {id_top_table_name, "id_top_table_name"},
        {id_column_name, "id_column_name"},
        {id_table_name, "id_table_name"},
        {id_create_table_name, "id_create_table_name"},
        {id_create_column_name, "id_create_column_name"},

        {id_schema_name, "id_schema_name"},
        {id_pragma_name, "id_pragma_name"},
        {id_pragma_value, "id_pragma_value"},

        {id_index_name, "id_index_name"},
        {id_trigger_name, "id_trigger_name"},
        {id_module_name, "id_module_name"},

        {id_window_def_name, "id_window_def_name"},
        {id_window_name, "id_window_name"},

        {id_window_base_name, "id_window_base_name"},
        {id_savepoint_name, "id_savepoint_name"},
        {id_collation_name, "id_collation_name"},
        {id_database_name, "id_database_name"},

        {id_references_table_name, "id_references_table_name"},
        {id_references_column_name, "id_references_column_name"},
        {id_foreignkey_column_name, "id_foreignkey_column_name"},
    };

    auto it = idtype_map.find(type);
    if (it != idtype_map.end()) {
        return it->second;
    } else {
