 ...
 ** Token Definition
 *********************************/
%token <sval> STRING IDENTIFIER ESCAPED_CURLY_LITERAL ESCAPED_BRACKET_LITERAL JSONPATH_STRING
%token <fval> FLOATVAL
%token <ival> INTVAL
...
/* For Queryhouse */
%token FOREIGN REFERENCES ACTION JSON STORED RECURSIVE
%token TRASH
%token ARROW
%token CUME_DIST DENSE_RANK PERCENT_RANK RANK ROW_NUMBER LAG LEAD NTILE FIRST_VALUE LAST_VALUE NTH_VALUE
...
%type <Column_In_Paren_ForeignKey_t> ColumnInParen_ForeignKey
%type <Column_In_Paren_References_t> ColumnInParen_References
...
%type <Jsonexpr_all_t> Jsonexpr_all
%type <Jsonpath_t> Jsonpath
%type <Jsondata_t> Jsondata
%type <json_literal_t> json_literal
%type <json_pathliteral_t> json_pathliteral
%type <opt_generatedarg_t> opt_generatedarg
%type <WithRecursiveClause_t> WithRecursiveClause
%type <json_expr_t> json_expr
%type <with_rec_desc_list_t> with_recursive_description_list
%type <with_recursive_description_t> with_recursive_description
%type <with_recursive_select_t> with_recursive_select
%type <init_select_t> init_select
%type <opt_recursive_select_t> opt_recursive_select
%type <select_list_essential_alias_t> select_list_essential_alias
%type <expr_list_essential_alias_t> expr_list_essential_alias
%type <expr_essential_alias_t> expr_essential_alias
%type <fts_query_literal_t> fts_query_literal
%type <window_with_order_t> window_with_order
%type <window_no_order_t> window_no_order
%type <order_t> order
%type <over_clause_t> over_clause
%type <over_clause_with_order_t> over_clause_with_order
%type <over_clause_no_order_t> over_clause_no_order
%type <window_func_no_arg_t> window_func_no_arg
%type <window_func_t> window_func
%type <aggregation_func_t> aggregation_func
%type <column_def_list_no_type_t> column_def_list_no_type
%type <column_def_no_type_t> column_def_no_type
%type <ColumnNameList_ForeignKey_t> ColumnNameList_ForeignKey
%type <ColumnNameList_References_t> ColumnNameList_References
%type <ColumnName_ForeignKey_t> ColumnName_ForeignKey
%type <ColumnName_References_t> ColumnName_References
 ...
 ******************************/
create_statement:
        CREATE opt_tmp TABLE opt_not_exists table_name FROM TBL FILE file_path {
            $$ = new CreateStatement();
            $$->sub_type_ = CASE0;
            $$->OptTmp_ = $2;
            $$->opt_not_exists_ = $4;
    ...
    |   CREATE opt_tmp TABLE opt_not_exists table_name '(' CreateInparan ')' {
            $$ = new CreateStatement();
            $$->sub_type_ = CASE1;
            $$->OptTmp_ = $2;
            $$->opt_not_exists_ = $4;
    ...
    |   CREATE opt_tmp TABLE opt_not_exists table_name AS select_statement {
            $$ = new CreateStatement();
            $$->sub_type_ = CASE2;
            $$->OptTmp_ = $2;
            $$->opt_not_exists_ = $4;
    ...
    |   CREATE VIEW table_name opt_column_list AS select_statement {
            $$ = new CreateStatement();
            $$->sub_type_ = CASE3;
            $$->table_name_ = $3;
            $$->table_name_->table_name_->id_type_ = id_create_table_name;
    ...
    |   CREATE VIRTUAL TABLE  opt_not_exists table_name USING module_name '(' column_def_list_no_type ')' {
            $$ = new CreateStatement();
            $$->sub_type_ = CASE6;
            $$->opt_not_exists_ = $4;
            $$->table_name_ = $5;
    ...
    ;
column_def_list_no_type:
        column_def_no_type { 
            $$ = new ColumnDefListNoType(); 
            $$->v_column_def_no_type_list_.push_back($1);
        }
    |   column_def_list_no_type ',' column_def_no_type { 
            $1->v_column_def_no_type_list_.push_back($3);
            $$ = $1;
        }
    ;
...
ForeignKeyColumn:
    FOREIGN KEY ColumnInParen_ForeignKey {
        $$ = new ForeignKeyColumn();
        $$->column_name_ForeignKey_ = $3;
    }
    ;

ForeignKeyRef:
    REFERENCES table_name ColumnInParen_References {
        $$ = new ForeignKeyRef();
        $$->table_name_ = $2;
        $$->column_name_References_ = $3;
        $$->table_name_->table_name_->id_type_ = id_references_table_name;
    }
    ;
...
ColumnInParen_ForeignKey:
       '(' ColumnNameList_ForeignKey ')' {
        $$ = new ColumnInParen_ForeignKey();
        $$->column_name_list_ForeignKey_ = $2;
    }
    ;

ColumnNameList_ForeignKey:
        ColumnName_ForeignKey {
            $$ = new ColumnNameList_ForeignKey();
            $$->v_column_name_list_ForeignKey_.push_back($1);
        }
    |   ColumnNameList_ForeignKey ',' ColumnName_ForeignKey {
            $1->v_column_name_list_ForeignKey_.push_back($3);
            $$ = $1;
        }
    ;

ColumnName_ForeignKey:
        IDENTIFIER { $$ = new ColumnName_ForeignKey(); $$->sub_type_=CASE0; $$->identifier_=new Identifier($1, id_foreignkey_column_name); free($1);}
    ;


ColumnInParen_References:
       '(' ColumnNameList_References ')' {
        $$ = new ColumnInParen_References();
        $$->column_name_list_References_ = $2;
    }
    ;

ColumnNameList_References:
        ColumnName_References {
            $$ = new ColumnNameList_References();
            $$->v_column_name_list_References_.push_back($1);
        }
    |   ColumnNameList_References ',' ColumnName_References {
            $1->v_column_name_list_References_.push_back($3);
            $$ = $1;
        }
    ;

ColumnName_References:
        IDENTIFIER { $$ = new ColumnName_References(); $$->sub_type_=CASE0; $$->identifier_=new Identifier($1, id_references_column_name); free($1);}
    ;
...
column_arg:
    ...
    |   GENERATED ALWAYS AS '(' expr ')' opt_generatedarg {
                $$ = new ColumnArg(); 
                $$->sub_type_ = CASE4; 
                $$->expr_ = $5;
                $$->opt_generatedarg_ = $7;
...
opt_generatedarg:
        STORED { $$ = new opt_generatedarg(); $$->sub_type_ = CASE0; $$->str_val_ = string("STORED"); }
    |   VIRTUAL { $$ = new opt_generatedarg(); $$->sub_type_ = CASE1; $$->str_val_ = string("VIRTUAL"); }
    |   /* empty */ { $$ = new opt_generatedarg(); $$->sub_type_ = CASE2; $$->str_val_ = string(""); }
...
column_type:
    ...
    |   JSON { $$ = new ColumnType(); $$->str_val_ = string("JSON"); }
...
window:
        window_with_order { $$ = new Window(); $$->sub_type_ = CASE0; $$->WindowWithOrder_ = $1; }
    |   window_no_order{ $$ = new Window(); $$->sub_type_ = CASE1; $$->WindowNoOrder_ = $1; }
    ;

window_with_order:
        opt_base_window_name PARTITION BY expr_list order opt_frame {
            $$ = new WindowWithOrder();
            $$->sub_type_ = CASE0;
    ...
    |   opt_base_window_name order opt_frame {
            $$ = new WindowWithOrder();
            $$->sub_type_ = CASE1;
            $$->opt_base_window_name_ = $1;
            $$->order_ = $2;
...
window_no_order:
        opt_base_window_name PARTITION BY expr_list opt_frame {
            $$ = new WindowNoOrder();
            $$->sub_type_ = CASE0;
            $$->opt_base_window_name_ = $1;
            $$->expr_list_ = $4;
    ...
    |   opt_base_window_name opt_frame {
            $$ = new WindowNoOrder();
            $$->sub_type_ = CASE1;
            $$->opt_base_window_name_ = $1;
            $$->opt_frame_ = $2;
...
opt_order:
        order { $$ = new OptOrder(); $$->sub_type_ = CASE0; $$->order_ = $1; }
    |   /* empty */ {  $$ = new OptOrder(); $$->sub_type_ = CASE1; }
    ;

order: 
        ORDER BY order_list { $$ = new Order(); $$->order_list_ = $3; }
    ;
...
operand: 
    ...
    |   json_expr   { $$ = new Operand(); $$->sub_type_ = CASE1; $$->expr_ = $1; }
    ;
...
binary_expr:
    ...
    |   operand GLOB operand    { $$ = new BinaryExpr(); $$->sub_type_ = CASE2; $$->operand1_ = $1; $$->operand2_ = $3; $$->operator_ = string("GLOB");}
    |   column_name MATCH fts_query_literal   {
            $$ = new BinaryExpr(); 
            $$->sub_type_ = CASE2; 
            $$->column_name_ = $1; 
            $$->FtsQueryLiteral_ = $3;
...
fts_query_literal:
	STRING { $$ = new FtsQueryLiteral(); $$->str_val_=$1; free($1);} 
	;
...
function_expr: 
    ...
    |   aggregation_func '(' ')' opt_filter_clause opt_over_clause {
            $$ = new FunctionExpr(); 
            $$->sub_type_ = CASE4; 
            $$->AggregationFunc_ = $1;
            $$->opt_filter_clause_ = $4;
    ...
    |   aggregation_func '(' opt_distinct expr_list ')' opt_filter_clause opt_over_clause {
            $$ = new FunctionExpr(); 
            $$->sub_type_ = CASE5; 
            $$->AggregationFunc_ = $1;
            $$->opt_distinct_ = $3;
    ...
    |   window_func_no_arg '(' ')' over_clause_with_order {
            $$ = new FunctionExpr();
            $$->sub_type_ = CASE2;
            $$->WindowFuncNoArg_ = $1;
            $$->OverClauseWithOrder_ = $4;
        }
    |   window_func '(' expr_list ')' over_clause {
            $$ = new FunctionExpr();
            $$->sub_type_ = CASE3;
            $$->WindowFunc_ = $1;
            $$->expr_list_ = $3;
    ...
    /*
    |   aggregation_func '(' ')' opt_filter_clause over_clause_with_order {
            $$ = new FunctionExpr();
            $$->sub_type_ = CASE4;
            $$->AggregationFunc_ = $1;
            $$->opt_filter_clause_ = $4;
            $$->OverClauseWithOrder_ = $5;
        }
    |   aggregation_func '(' opt_distinct expr_list ')' opt_filter_clause over_clause_with_order {
            $$ = new FunctionExpr(); 
            $$->sub_type_ = CASE5; 
            $$->AggregationFunc_ = $1; 
            $$->opt_distinct_ = $3;
            $$->expr_list_ = $4;
            $$->opt_filter_clause_ = $6;
            $$->OverClauseWithOrder_ = $7;
        }
    */
    ;
    
aggregation_func:
        AVG { $$ = new AggregationFunc(); $$->str_val_ = string("AVG"); }
    |   MAX { $$ = new AggregationFunc(); $$->str_val_ = string("MAX"); }
    |   MIN { $$ = new AggregationFunc(); $$->str_val_ = string("MIN"); }
    |   SUM { $$ = new AggregationFunc(); $$->str_val_ = string("SUM"); }
    |   COUNT { $$ = new AggregationFunc(); $$->str_val_ = string("COUNT"); }
    ;

window_func_no_arg:
        CUME_DIST { $$ = new WindowFuncNoArg(); $$->str_val_ = string("CUME_DIST"); }
    |   DENSE_RANK { $$ = new WindowFuncNoArg(); $$->str_val_ = string("DENSE_RANK"); }
    |   PERCENT_RANK { $$ = new WindowFuncNoArg(); $$->str_val_ = string("PERCENT_RANK"); }
    |   RANK { $$ = new WindowFuncNoArg(); $$->str_val_ = string("RANK"); }
    |   ROW_NUMBER { $$ = new WindowFuncNoArg(); $$->str_val_ = string("ROW_NUMBER"); }
    ;

window_func:
        LAG { $$ = new WindowFunc(); $$->str_val_ = string("LAG"); }
    |   LEAD { $$ = new WindowFunc(); $$->str_val_ = string("LEAD"); }
    |   NTILE { $$ = new WindowFunc(); $$->str_val_ = string("NTILE"); }
    |   FIRST_VALUE { $$ = new WindowFunc(); $$->str_val_ = string("FIRST_VALUE"); }
    |   LAST_VALUE { $$ = new WindowFunc(); $$->str_val_ = string("LAST_VALUE"); }
    |   NTH_VALUE { $$ = new WindowFunc(); $$->str_val_ = string("NTH_VALUE"); }
    ;



opt_over_clause:
        over_clause { $$ = new OptOverClause(); $$->sub_type_ = CASE0; $$->OverClause_ = $1; }
    |   /* emtpy */ {$$ = new OptOverClause(); $$->sub_type_ = CASE1; }
    ;
...
over_clause:
        over_clause_with_order {
            $$ = new OverClause();
            $$->sub_type_ = CASE0;
            $$->OverClauseWithOrder_ = $1;
        }
    |   over_clause_no_order {
            $$ = new OverClause();
            $$->sub_type_ = CASE1;
            $$->OverClauseNoOrder_ = $1;
        }
...
over_clause_with_order:
        OVER '(' window_with_order ')' {$$ = new OverClauseWithOrder(); $$->WindowWithOrder_ = $3;}
    ;

over_clause_no_order:
        OVER '(' window_no_order ')' {$$ = new OverClauseNoOrder(); $$->WindowNoOrder_ = $3;}
    ;
...
json_literal:
        ESCAPED_CURLY_LITERAL { $$ = new json_literal(); $$->str_val_ = $1; free($1); }
    |   ESCAPED_BRACKET_LITERAL { $$ = new json_literal(); $$->str_val_ = $1; free($1); }
    ;

json_pathliteral:
        ARROW JSONPATH_STRING {  $$ = new json_pathliteral(); $$->str_val_ = $2; free($2);  }
    ;
...
opt_with_clause:
    ...
    |   WithRecursiveClause { $$ = new OptWithClause(); $$->sub_type_ = CASE1; $$->WithRecursiveClause_ =$1; }
    |   /* empty */ { $$ = new OptWithClause(); $$->sub_type_ = CASE2; }
    ;
...
WithRecursiveClause:
        WITH RECURSIVE with_recursive_description_list { $$ = new WithRecursiveClause(); $$->with_rec_desc_list = $3; }
    ;

with_recursive_description_list:
        with_recursive_description {
            $$ = new WithRecDescList();
            $$->v_with_rec_desc_list.push_back($1);
        }
    |   with_recursive_description_list ',' with_recursive_description
     {
            $1->v_with_rec_desc_list.push_back($3);
            $$ = $1;
    }
    ;

with_recursive_description:
        IDENTIFIER AS with_recursive_select {
            $$ = new WithRecDesc();
            $$->id_ = new Identifier($1);
            $$->with_recursive_select_ = $3;
            free($1);
        }
    ;

with_recursive_select:
        '(' init_select opt_recursive_select ')' {
            $$ = new WithRecursiveSelect();
            $$->init_select_ = $2;
            $$->opt_recursive_select_ = $3;
        }
    ;

opt_recursive_select:
        UNION ALL SELECT opt_distinct select_list opt_from_clause opt_where opt_group opt_order LIMIT INTVAL {
            $$ = new OptRecursiveSelect();
            $$->sub_type_ = CASE0;
            $$->opt_distinct_ = $4;
            $$->select_list_ = $5;
            $$->opt_from_clause_ = $6;
    ...
    |   /* empty */  {$$ = new OptRecursiveSelect(); $$->sub_type_ = CASE1;}
    ;

init_select:
        SELECT opt_distinct select_list_essential_alias opt_from_clause opt_where opt_group {
            $$ = new InitSelect();
            $$->opt_distinct_ = $2;
            $$->select_list_essential_alias_ = $3;
            $$->opt_from_clause_ = $4;
            $$->opt_where_ = $5;
            $$->opt_group_ = $6;
        }
...
select_list_essential_alias:
        expr_list_essential_alias {
            $$ = new SelListAlias();
            $$->expr_list_essential_alias_ = $1;
        }
    ;

expr_list_essential_alias:
        expr_essential_alias { 
            $$ = new ExprListAlias(); 
            $$->v_expr_list_essential_alias_.push_back($1); 
            }
    |   expr_list_essential_alias ',' expr_essential_alias { 
        $1->v_expr_list_essential_alias_.push_back($3); 
        $$ = $1; }
    ;

expr_essential_alias:
        expr alias {
            $$ = new ExprEssentialAlias();
            $$->expr_ = $1;
            $$->alias_ = $2;
        }
    ;
